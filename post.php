        ...
        <div class="row mt-4">
            ...
            FROM post INNER JOIN user ON (post.user_id = user.id) WHERE post.id = $_GET[id]";
                ...
                while ($row = $result->fetch()) {
                    ...
                    echo "<div class='card-body'>$row[1]<br> $row[3] - $row[2]
                </div>";
                    echo "</div><br>";
                }
                $sql = "SELECT comment.content,comment.post_date,user.login,comment.id
            FROM comment INNER JOIN user ON (comment.user_id = user.id) WHERE comment.post_id = $_GET[id]";
                ...
                while ($row = $result->fetch()) {
                    ...
                    echo "<div class='card-body'>$row[0]<br> $row[2] - $row[1]
                        </div>";
                    echo "</div><br>";
                    $i = $i + 1;
                }
